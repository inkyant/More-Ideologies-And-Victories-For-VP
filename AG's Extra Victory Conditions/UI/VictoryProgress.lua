print("Loading VictoryProgress.lua from AG EVC")
----------------------------------------------------------------
----------------------------------------------------------------
include( "IconSupport" );
...
local g_ReligiousIM = InstanceManager:new( "ReligiousCiv", "Civ", Controls.ReligiousCompareStack );
...
function PopulateReligious()
	...
	if(xTest1 == 1) then
		...
		-- Accumulated Faith
		local iPlayerFaith = g_EVCReligiousFaith[iActivePlayer];
		local bDone = false;
		if iPlayerFaith >= g_EVCFaithTarget then
			bDone = true;
		end
		local faithStatusKey = bDone and "TXT_KEY_VP_RELIGIOUS_FAITH_DONE" or "TXT_KEY_VP_RELIGIOUS_FAITH";
		--Controls.ReligiousVictoryFaith:SetText(Locale.Lookup(faithStatusKey, iPlayerFaith, g_EVCFaithTarget));
		...
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
			...
			local pPlayer = Players[iPlayerLoop];
				...
				local victoryPercent = (spread/90);
				...
				local txt = Locale.Lookup(statusKey, religion.IconString, religion.Description, iSpread, pPlayer:GetNameKey(), pPlayer:GetFaith());
				item.IconFrame:SetToolTipString(txt);
...
function PopulateEconomic()
	...
	if(xTest2 == 1) then
		...
		-- Accumulated Gold
		local iGoldTarget = g_EVCGoldTarget;
		...
		print("We are accumulating gold!");
		...
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
			...
			--Create new row if one does not alread exist.
				...
				--Get global variables concerning economic victory and progress
				local iNumMonopoly = 0;
				if(g_EVCPlayerNumMonopoly ~= nil) then
					if(g_EVCPlayerNumMonopoly[iPlayer] ~= nil) then
						iNumMonopoly = g_EVCPlayerNumMonopoly[iPlayer];
					end
				end

				local iNumMonopolyTarget = 0;
				if(g_EVCMonopolyTarget ~= nil) then
					iNumMonopolyTarget = g_EVCMonopolyTarget;
				end

				-- Calculate victory progress bar size
				local NumMon = iNumMonopoly;
				if iNumMonopoly >= iNumMonopolyTarget then
					NumMon = iNumMonopolyTarget;
				end

				local iNumFranchise = 0;
				if(g_EVCFranchises ~= nil) then
					iNumFranchise = g_EVCFranchises[iPlayer];
				end
				local victoryPercent = (NumMon/iNumMonopolyTarget);
				...
				local iPlayerIcon = bHasMet and iPlayer or -1;
				local MonopolyKey = bHasMet and "TXT_KEY_VP_ECONOMIC_MONOPOLY" or "TXT_KEY_VP_ECONOMIC_MONOPOLY_UNMET";
				...
				local txt = Locale.Lookup(MonopolyKey, pPlayer:GetNameKey(), iNumMonopoly, iNumMonopolyTarget, pPlayer:GetGold(), iGoldTarget);
				item.IconFrame:SetToolTipString(txt);
	...
	Controls.EconomicStack:CalculateSize();
	Controls.EconomicStack:ReprocessAnchoring();
...
print("OK loaded VictoryProgress.lua from AG EVC")